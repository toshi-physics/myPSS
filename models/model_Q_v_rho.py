...
def main():
    ...
    initParser = argparse.ArgumentParser(description='Run Model_Q_v_rho')
    ...
    savedir     = "data/model_Q_v_rho/gamma0_{:.2f}_rhoseed_{:.2f}_pi_{:.3f}/run_{}/".format(Gamma0, rho_seed, Pii, run)
    ...
    # Define vx
    system.create_term("vx", [('iqxQxx', None), ("rho", (np.power, -1))], [alpha/gamma, 0, 0, 0, 0])
    system.create_term("vx", [('iqyQxy', None), ("rho", (np.power, -1))], [alpha/gamma, 0, 0, 0, 0])
    system.create_term("vx", [('iqxp', None), ("rho", (np.power, -1))], [-1/gamma, 0, 0, 0, 0])
    # Define vy
    system.create_term("vy", [('iqxQxy', None), ("rho", (np.power, -1))], [alpha/gamma, 0, 0, 0, 0])
    system.create_term("vy", [('iqyQxx', None), ("rho", (np.power, -1))], [-alpha/gamma, 0, 0, 0, 0])
    system.create_term("vy", [('iqyp', None), ("rho", (np.power, -1))], [-1/gamma, 0, 0, 0, 0])
    # Define kappa_a_xy
    ...
    for t in tqdm(range(n_steps)):
        ...
        if t % dn_dump == 0:
            ...
            np.savetxt(savedir+'vx.csv.'+ str(t), vx.get_real(), delimiter=',')
            np.savetxt(savedir+'vy.csv.'+ str(t), vy.get_real(), delimiter=',')
